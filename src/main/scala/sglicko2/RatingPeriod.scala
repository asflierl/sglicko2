...
object RatingPeriod:
  def empty[A: Eq, B](using ScoringRules[A, B]): RatingPeriod[A, B] = new RatingPeriod(Map.empty)

  def apply[A: Eq, B](gamesToAdd: B*)(using ScoringRules[A, B]): RatingPeriod[A, B] =
